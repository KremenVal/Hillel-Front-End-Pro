...
window.onkeydown = function(event) {
	if (!box.classList.contains('animation-box')) {
...
function getStep(nameVariable) {
	let variable = prompt('Enter a number for the ' + nameVariable + ':', 5)
		maxStep = monitorParams.clientHeight / 3;

	while (!isNumber(variable) || parseInt(variable) < 1 || parseInt(variable) > maxStep) {
		console.log(isNumber(variable));
		variable = prompt(`You entered no number or number less than 1 or bigger than ${maxStep}. Need to enter a valid number for ` + nameVariable, 5);
	}
